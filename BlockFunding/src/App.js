...
import contract from "./artifacts/contracts/BlockFunding.sol/BlockFunding.json";
...
const App = () => {
    ...
    const crowdFund = async () => {
        try {
            const contractInstance = web3.contract.connect(web3.signer);
            const fund = toString(fundCount);
            // console.log(fundCount)
            await contractInstance.fundPetitions({
                
                value: ethers.utils.parseEther(fundCount),
    ...
    useEffect(() => {
        const getManager = async () => {
            const promise = web3.contract.attach(contractAddress).manager()
            console.log(web3.contract.attach(contractAddress).manager())
            promise.then((result) =>{
                setManager(result)
            })
        };
        web3.contract && getManager();
    }, [web3.contract, refresh]);
                ...
                >
                    ...
                    <Tab eventKey="third" title="募捐列表" className="tabtitle">
                        {listPetition.length > 0 ? (
                            ...
                            <Button className="finalbutton">目前没有募捐</Button>
                        )}
                    </Tab>
                                ...
                                <Form>
                                    <Form.Floating className="mb-3 form1" >
                                        <Form.Control
                                            ...
                                            name="fundCount"
                                            value={fundCount}
                                            onChange={handleFundCount}
                                            required
                                        />
                                        <label htmlFor="floatingInputCustom">捐出SepoliaETH的数量（需大于0.01）</label>
                                    </Form.Floating>
                                </Form>
                                <Button
